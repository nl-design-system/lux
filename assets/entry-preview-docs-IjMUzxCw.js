import { a as _baseGetTag, b as _getPrototype, c as isObjectLike_1, d as isArray_1, $ as $w, z as zw, e as eP, O, s as sT, f as isFunction, K as Ki, B as Bw, P as Pm, g as $i, o as os, H as Ht } from "./index-DtSuK_9N.js";
...
function generateObject({ inferredType, ast: ast2 }) {
  ...
  return O(OBJECT_CAPTION, generateObjectCode(ast2));
}
function generateArray2({ inferredType, ast: ast2 }) {
  let { depth } = inferredType;
  if (depth <= 2) {
    let compactArray = generateArrayCode(ast2, true);
    if (!os(compactArray)) return O(compactArray);
  }
  return O(ARRAY_CAPTION, generateArrayCode(ast2));
}
...
function generateFunc2({ inferredType, ast: ast2 }) {
  let { identifier } = inferredType;
  if (identifier != null) return O(getPrettyIdentifier(inferredType), generateCode(ast2));
  let prettyCaption = generateCode(ast2, true);
  return os(prettyCaption) ? O(FUNCTION_CAPTION, generateCode(ast2)) : O(prettyCaption);
}
...
}, defaultResolver = (rawDefaultProp) => O(rawDefaultProp.toString()), DEFAULT_TYPE_RESOLVERS = { string: stringResolver, object: objectResolver, function: functionResolver, default: defaultResolver };
...
function getPropDefs(component, section) {
  ...
  !Ki(component) && !component.propTypes && isMemo(component) && (processedComponent = component.type);
...
var parameters = { docs: { story: { inline: true }, extractArgTypes, extractComponentDescription: $w } }, decorators = [jsxDecorator], argTypesEnhancers = [zw];
export {
  applyDecorators,
  argTypesEnhancers,
  decorators,
